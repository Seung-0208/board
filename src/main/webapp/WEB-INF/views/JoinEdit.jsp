...
<html>
      ...
      <nav class="navbar navbar-light bg-light fixed-top">
        ...
        <div class="align-items-center" style="padding-top: 200px; text-align: center;">
            ...
            <div class="gap-1" style="display:flex; flex-direction: column;align-items: center;">
              <div style="display:flex; padding-top: 30px; padding-bottom: 70px;">등록된 회원 정보를 수정하는 페이지 입니다</div>
    ...
    <div>
      <form class="needs-validation" novalidate method="post" action="<c:url value="/Proj/JoinEdit.ict" />" >
        <fieldset>
          <legend class="my-4"><span class="number">1</span> Your Basic Info</legend>
          <div class="my-4">
            <label>Name:</label>
            <p style="font-size: small;">다른 이용자에게 보여줄 이름을 설정해주세요</p>
            <input id="writer" class="form-control" type="text" name="name" required value="${record.name }">
              <div class="invalid-feedback">이름은 적어도 한 글자 이상 작성해야 합니다</div>
          </div>
          <div class="row align-items-start"> <!--로그인-->
            <div class="col" id="idDiv">
              <label style="color:#ccc">ID: 아이디는 수정할 수 없습니다</label>
              <input disabled=true id="nick" class="form-control" type="text" name="id" required value="${record.username }"> <!-- 유효성검사 테스트용 id -->
            ...
            <div class="col">
              ...
              <input id="pwd" class=" form-control" type="password" name="pwd" required value="${record.password }">
        ...
        <fieldset>
          ...
          <div class="my-4">
            ...
            <textarea required id="selfIntro" name="selfIntro" class="form-control" rows="5">${record.selfIntroduce }</textarea>
          </div>
        </fieldset>
       
        <button type="submit" class="btn btn-success">Upload</button>
        <div style="display:flex;justify-content: center;">
        	<a href="javascript:isLeave()">회원을 탈퇴하시겠습니까?</a>
        </div>
       </form>
      </div>
        </div>
...
<script>
	//회원탈퇴용
	function isLeave(){
		if(confirm('탈퇴하시겠습니까? 기존에 작성했던 글이 모두 삭제됩니다')){
			window.onbeforeunload = function(e){};
			location.replace("<c:url value="/Proj/Service/Leave.ict" />");
		}
		window.onbeforeunload = function(e){};
	}
	
	var originId = document.querySelector('#nick').value;
	//값을 뿌려주기 위한 코드
	//관심사 체크박스
	<c:forEach var="inter" items="${fn:split(record.inters,',')}">
		document.querySelector('[value=${inter }]').checked=true;
	</c:forEach>
	//성별
	var genderRadio = document.querySelectorAll('[name=gender]');
	genderRadio.forEach(item=>{
		if(item.value==='${fn:toLowerCase(record.gender)}'){
			item.checked = true;
		}
	})
	//학력
	var eduSelect = document.querySelectorAll('option'); 
	eduSelect.forEach(item=>{
		if(item.value.includes("${record.education}")){
			item.selected = true;
		}
	})
	...
	form.addEventListener('input', e=>{
		...
		//console.log(!form.checkValidity() || !isChecked());
		console.log(isclickedIdCheck);
		...
		//submit.disabled = !form.checkValidity() || !isChecked();
		
	})
	console.log(document.querySelector('[type=submit]'));
	submit.onclick=()=>{
		window.onbeforeunload = function(e){};
		/*if (!isclickedIdCheck && originId != document.querySelector('#nick').value){
